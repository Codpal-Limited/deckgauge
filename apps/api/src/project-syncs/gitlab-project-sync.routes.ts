...
export function gitlabProjectSyncRoutes(deps: { prisma: PrismaClient; singleUser?: boolean }) {
  ...
  return async function plugin(app: FastifyInstance) {
    app.get('/project-syncs/gitlab', { config: { policy: ORG_VIEWER } }, async (req) =>
      service.list(requireOrganizationId(req)),
    );

    app.post('/project-syncs/gitlab', { config: { policy: ORG_MEMBER } }, async (req, reply) => {
    ...
    app.patch<{ Params: { id: string } }>('/project-syncs/gitlab/:id', { config: { policy: ORG_MEMBER } }, async (req, reply) => {
    ...
    app.delete<{ Params: { id: string } }>('/project-syncs/gitlab/:id', { config: { policy: ORG_MEMBER } }, async (req, reply) => {
      const params = z.object({ id: z.string().uuid() }).safeParse(req.params);
      if (!params.success) return reply.code(400).send({ error: params.error.flatten() });
    ...
    app.post('/project-syncs/gitlab/ensure', { config: { policy: ORG_MEMBER } }, async (req, reply) => {
      const body = z
