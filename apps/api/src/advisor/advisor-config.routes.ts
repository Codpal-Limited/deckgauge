...
export function advisorConfigRoutes({ prisma }: { prisma: PrismaClient }) {
  return async function (app: FastifyInstance) {
    ...
    app.get('/advisor/config', { config: { policy: ADMIN } }, async (_req, reply) => {
      ...
      const sourceLookupEnabled = await service.isSourceLookupEnabled();
     ...
     *
     * `ADMIN`, like every other route here: it changes what the Advisor may
     * read for every user of the instance.
     *
     ...
     */
    app.put('/advisor/config/source-lookup', { config: { policy: ADMIN } }, async (req, reply) => {
      ...
      const updated = await service.setSourceLookupEnabled(parsed.data.enabled);
      if (!updated) {
    ...
    app.put('/advisor/config', { config: { policy: ADMIN } }, async (req, reply) => {
      const parsed = advisorConfigSchema.safeParse(req.body);
      if (!parsed.success) {
        return reply.code(400).send({ error: 'invalid config', details: parsed.error.flatten() });
      }
      await service.saveConfig(parsed.data);
      return reply.code(204).send();
    });

    app.post('/advisor/config/test', { config: { policy: ADMIN } }, async (req, reply) => {
      const parsed = advisorConfigSchema.safeParse(req.body);
      if (!parsed.success) {
        return reply.code(400).send({ error: 'invalid config', details: parsed.error.flatten() });
      }
