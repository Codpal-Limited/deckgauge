...
import { focusResidueVerdicts, type DemoFocusVerdict } from './focus-seed.js';
...
export interface DemoDataset {
  ...
  /**
   * Verdicts for the tasks the CAPEX and rule tiers cannot reach. Computed
   * here rather than in the writer because it needs the generated ClickHouse
   * rows AND the board's projects, and this is the one place that holds both.
   */
  focusVerdicts: DemoFocusVerdict[];
}
...
/**
 * The description every seeded issue carries, and the ONE thing that makes two
 * demo issues distinguishable by CONTENT.
 *
 * Not decoration. `taskFingerprint` — the key `focus_verdicts` is stored under —
 * hashes `normaliseTitle(title) + ' ' + normaliseTitle(description)`, and
 * `normaliseTitle` STRIPS issue keys (`/\b[A-Za-z]+-\d+\b/g`). Every project is
 * named `${topic} (${jiraKey})` from a 26-entry topic list, so with an empty
 * description the 240 generated issues collapsed to **26 fingerprints**, the
 * largest bucket holding 16 tasks. Measured, not feared: seeding the Focus
 * residue's 8 fingerprints then attached a HUMAN verdict — which outranks the
 * CAPEX tier — to 65 tasks that resolve `class: 'A', source: 'CAPEX'` today.
 *
 * The sequence number survives normalisation (digits do; a `PROJ-123` token does
 * not) and the board name separates the two boards, so title + description is
 * unique per issue. It also fills a hole of its own: every demo item used to
 * open with an empty description.
 *
 * `focus-seed.test.ts` asserts the 240 fingerprints are distinct, so this
 * cannot quietly regress to a format that re-collapses them.
 */
function demoIssueDescription(jiraKey: string, boardName: string): string {
  // `PLT-137` -> `137`. Split rather than sliced: the project key length is
  // not fixed by anything.
  const sequence = jiraKey.slice(jiraKey.indexOf('-') + 1);
  return `Seeded demo work item ${sequence} on the ${boardName} board.`;
}
...
export function generateDemoDataset(now: Date, seed: number): DemoDataset {
  ...
  const clickhouse = generateClickHouse(rand, now, boards, employees);
  const projects = boards.flatMap((b) => b.projects);
  ...
  return {
    ...
    employees,
    clickhouse,
    focusVerdicts: focusResidueVerdicts({
      // The generated `jira_issues` rows, which are what the runtime reads.
      // Cast because `DemoClickHouse` types every table as a loose row bag; the
      // five fields named here are written a few hundred lines below and
      // `focus-seed.test.ts` exercises the same call against a real dataset.
      issues: clickhouse.jiraIssues as unknown as {
        key: string;
        summary: string;
        description: string;
        issue_type: string;
      ...
      // `Project.costClassification` by Jira key, which is what `loadCapex`
      // serves the classifier from Postgres.
      capexByKey: new Map(
        projects
          .filter((p) => p.costClassification !== null)
          .map((p) => [p.jiraKey, p.costClassification!] as const),
      ),
      // The denominator exactly as focus-data.service.ts derives it:
      // CAPEX-marked board rows whose type is Epic. Both halves matter — an
      // OPEX or unclassified epic is not roadmap scope, and a CAPEX Story is
      // not an epic.
      roadmapEpicKeys: new Set(
        projects
          .filter((p) => p.costClassification === 'CAPEX' && p.jiraType === 'Epic')
          .map((p) => p.jiraKey),
      ),
    }),
  };
...
/**
 * The demo's Jira status vocabulary, and the source `jira_issues.status` /
 * `status_category` are written from.
 *
 * Exported because Team Focus's `workingStates` is DERIVED from it rather than
 * restated — see `focus-seed.ts`. It agrees with `CATALOG_STATUSES` (the board
 * `BoardStatus` labels) and `focus-seed.test.ts` asserts that they do, because
 * nothing else did.
 */
  ...
  for (const board of boards) {
    ...
    for (const project of board.projects) {
      ...
      out.jiraIssues.push({
        ...
        description: demoIssueDescription(key, board.name),
